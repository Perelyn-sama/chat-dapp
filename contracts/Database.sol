...
contract Database {
    ...
    // Stores the default name of an user and their friends info
    struct user {
        string name;
        friend[] friendList;
    }
    ...
    // If checks whether a user(identified by its public key)
    ...
    function getUsename(address pubkey) external view returns(string memory){
    ...
    // Check if two users are already freinds or not 
    ...
    // A helper functio to update the friendlist 
